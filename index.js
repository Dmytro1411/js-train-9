...
function findElementGreaterThan(arr, num) {
  ...
  let foundElement = arr.find((item) => item > num);
...
function findLastEvenNumber(arr) {
  ...
  let lastArr = arr.findLast((item) => item % 2 === 0);
  return lastArr;
}
...
function getPositiveArrayValues(arr) {
  ...
  let positive = arr.values();
  let positiveArray = Array.from(positive);
  //   log(positiveArray);
  let positiveArrayValues = positiveArray.filter((values) => values > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  let arrSpice = arr.splice(0, startIdx);
  //   log(arrSpice);
  let doubledArray = arrSpice.map((user) => user * 2);
  return doubledArray;
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  //   let squaredArray = a.map((item) => item * item);
  return squaredArray;
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let sum = arr.slice(-n).reduceRight((num, user) => {
    return user + num;
  });

  arr.slice(-n).forEach((item) => {
    log(item);
  });
  return sum;
}
...
function wordLengths(arr) {
  ...
  let lengthsArray = arr.map((item) => (item = item.length));
  return lengthsArray;
}
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) || typeof n !== "number") {
    ...
    let isArr = (item) => typeof item !== "number";
    if (arr.some(isArr) === true) {
      return "Перший вхідний параметр має бути масивом, другий - числом";
    } else {
      log("Перевірку пройдено! Все добре. Можна продовжувати працювати");
    }
  }
...
function findLongestWord(arr) {
  ...
  let longestWord = arr.reduce((num, user) => {
    // log(user.length, num);
    // log(num);
    return user.length > num.length ? (num = user) : num;
  });
  return longestWord;
}
...
function findDuplicateElements(arr) {
  ...
  //   let duplicateElements = (arr) => {
  //     return arr.filter((item, id) => arr.indexOf(item) !== id);
  //   };
  //   return duplicateElements(arr);
  if (!Array.isArray(arr)) {
    return [];
  }
...
// let s = (arr) => arr.join().charAt(0).toUpperCase() + arr.slice(1);
// log(s(arr));
...
function capitalizeWords(arr) {
  ...
  let capitalizedArray = (item) =>
    item.toString().charAt(0).toUpperCase() + item.slice(1);
  return arr.map(capitalizedArray);
...
function calculateTotalPrice(arr) {
  ...
  let totalPrice = arr.reduce((num, user) => num + user.price, 0);
  return totalPrice;
}
